//
//  TBAnimatedTabBarController.swift
//  uktcShedule
//
...
class TBAnimatedTabBarItem: UITabBarItem {

    @IBOutlet weak var animation: TBItemAnimation?
...
// MARK: create methods
    ...
    func createCustomIcons(containers : [String: UIView]) {
        ...
        if let items = tabBar.items as? [TBAnimatedTabBarItem] {
...
// MARK: actions
    ...
    func tapHandler(gesture:UIGestureRecognizer) {
        ...
        let items = tabBar.items as! [TBAnimatedTabBarItem]

        let currentIndex = gesture.view!.tag
        if selectedIndex != currentIndex {
            let animationItem : TBAnimatedTabBarItem = items[currentIndex]
